...
### リンク検証（投稿前に必ず実施）
記事内のすべてのリンクを投稿前にHTTPステータスで検証する。
1. **内部リンク**: 関連記事リンクのURLにfetch/curlでアクセスし、200が返ることを確認。404の場合は修正または削除
2. **参考文献リンク**: DOI・外部URLにアクセスし、リダイレクト先が有効であることを確認
3. **リンクなし参考文献**: PubMed/WebSearchでURL（DOI・公式サイト）を検索し、可能な限りリンクを付与
4. **YouTube概要欄のブログURL**: はてな投稿後にURLが確定するため、投稿後に概要欄のURLを更新
