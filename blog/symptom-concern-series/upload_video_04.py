...
- ローカル 04_facial_twitch_hatena.html (埋め込み挿入済み) を丸ごとAtomPub PUT
...
def main():
    ...
    if "<!-- youtube embed -->" not in html or VIDEO_ID not in html:
        raise RuntimeError("ローカルHTMLにiframe埋め込み（VIDEO_ID={}）が見つかりません。先に埋め込みを挿入してください。".format(VIDEO_ID))
