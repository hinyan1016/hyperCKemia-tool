#!/usr/bin/env python3
"""
その症状大丈夫#06 自律神経 — 本番公開記事へ
- インフォグラフィックを Fotolife にアップロード
- 06_autonomic_nerves_hatena.html に「インフォグラフィック / YouTube iframe / VideoObject JSON-LD」を挿入
...
IMAGE_FILE = Path(r"C:\Users\jsber\OneDrive\Documents\Claude_task\blog\symptom-concern-series\infographics\06_autonomic_nerves.png")
HTML_FILE = Path(r"C:\Users\jsber\OneDrive\Documents\Claude_task\blog\symptom-concern-series\06_autonomic_nerves_hatena.html")

IMAGE_ALT = (
    "自律神経失調症と言われたら まとめインフォグラフィック — "
    "①自律神経の役割（交感↔副交感）／②よくある症状＋仮の診断名／"
    "③隠れた病気6つ（甲状腺疾患・貧血・うつ病・起立性調節障害・不整脈・副腎不全）／"
    "④まず受けるべき5つの検査／⑤危険なサイン5つ／⑥受診先の選び方／"
    "⑦自律神経を整える5つの習慣／3つのまとめポイント"
)
IMAGE_TITLE = "autonomic_nerves_infographic"

TITLE_KEYWORD = "自律神経失調症"
TARGET_URL_FRAGMENT = "2026/04/27/212110"
REQUIRE_DRAFT = False  # 公開済み記事を対象

VIDEO_ID = "6KkLHc5jnKM"
VIDEO_TITLE = "「自律神経失調症」と言われたら？ ― 本当の原因と正しい受診先を脳神経内科医が解説"
VIDEO_DURATION_MIN = "約8分"
VIDEO_UPLOAD_DATE = "2026-04-27T00:00:00+09:00"
VIDEO_DESCRIPTION = (
    "「自律神経失調症」と言われたけど本当に大丈夫？脳神経内科医が、自律神経の役割（交感神経・副交感神経）、よくある症状、"
    "隠れた病気6つ（甲状腺疾患・貧血・うつ病・起立性調節障害・不整脈・副腎不全）、まず受けるべき5つの検査、"
    "症状で選ぶ診療科、見逃せない危険なサイン、自律神経を整える5つの習慣（規則正しい睡眠・有酸素運動・"
    "ぬるめの入浴・腹式呼吸・デジタルデトックス）を解説します。"
)
...
def build_blocks(image_url):
    ...
    yt_block = (
        ...
        "\n"
        '<p style="background:#f7f9fc; border:1px solid #dfe6e9; padding:14px 16px; border-radius:6px;">'
        "<strong>📺 動画でも解説しています（{dur}）</strong><br>"
        "本記事の内容を動画でまとめています。音声で学びたい方はぜひご覧ください。<br>"
        '<a href="https://youtu.be/{vid}">▶ YouTubeで見る</a></p>\n'
    ).format(vid=VIDEO_ID, title=VIDEO_TITLE, dur=VIDEO_DURATION_MIN)
    ...
    jsonld_block = (
        ...
        "</script>\n"
    ).format(name=VIDEO_TITLE, desc=VIDEO_DESCRIPTION, vid=VIDEO_ID, date=VIDEO_UPLOAD_DATE)

    return img_block + yt_block + jsonld_block
...
def main():
    ...
    if "<!-- youtube embed -->" in html and VIDEO_ID in html and "<!-- infographic -->" in html:
        print("[4/4] 既に3ブロック挿入済み。そのままPUTします")
    else:
        # 目次の直前に3ブロックまとめて挿入
        target = "<!-- 目次 -->"
        if target not in html:
            raise RuntimeError("挿入先（<!-- 目次 -->）が見つかりません")
        blocks = build_blocks(image_url)
        html = html.replace(target, blocks + "\n" + target, 1)
        HTML_FILE.write_text(html, encoding="utf-8")
        print("[4/4] ローカルHTMLに3ブロック挿入完了（{}バイト）".format(len(html.encode("utf-8"))))
