...
- 01_hand_tremor_hatena.htmlに画像タグを挿入（リード文直後、目次前）
...
IMAGE_FILE = Path(r"C:\Users\jsber\OneDrive\Documents\Claude_task\youtube-slides\その症状大丈夫01_手のふるえ\infographic.png")
HTML_FILE = Path(r"C:\Users\jsber\OneDrive\Documents\Claude_task\blog\symptom-concern-series\01_hand_tremor_hatena.html")

IMAGE_ALT = "手のふるえ（振戦）の原因と見分け方 インフォグラフィック — 3つのタイプ・パーキンソン病と本態性振戦の比較・危険なサイン5つ・受診の目安"
TITLE_KEYWORD = "これってパーキンソン病"
REQUIRE_DRAFT = True
...
def upload_fotolife():
    ...
    xml += '  <title>hand_tremor_infographic</title>\n'
...
def find_entry_id():
    ...
    while url and page < 30:
        ...
        page += 1
    raise RuntimeError("対象記事が見つかりません（TITLE_KEYWORD={}, REQUIRE_DRAFT={}）".format(TITLE_KEYWORD, REQUIRE_DRAFT))
...
def main():
    ...
    if marker in html:
        ...
        # リード文2つの直後（目次<ul class="table-of-contents">の直前）に挿入
        target = '<ul class="table-of-contents">'
        if target not in html:
            raise RuntimeError("挿入先（目次ul）が見つかりません")
