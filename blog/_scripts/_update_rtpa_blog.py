...
def convert_to_inline(html):
    ...
    content = re.sub(r'<h1([^>]*)>', r'<h1\1 style="font-size:1.6rem;color:#2c5aa0;border-bottom:2px solid #2c5aa0;padding-bottom:8px;margin-top:0;">', content)
    content = re.sub(r'<h2([^>]*)>', r'<h2\1 style="font-size:1.3rem;color:#2c5aa0;border-left:4px solid #2c5aa0;padding-left:12px;margin-top:1.5rem;">', content)

    def replace_h3(m):
        return '<h3{} style="font-size:1.1rem;color:#444;margin-top:1rem;">\u25B6 '.format(m.group(1))
    content = re.sub(r'<h3([^>]*)>', replace_h3, content)

    content = re.sub(r'<strong>', '<strong style="font-weight:bold;color:#2c5aa0;">', content)
    content = re.sub(r'<sup>', '<sup style="color:#dc3545;font-weight:bold;font-size:0.75em;">', content)
    ...
    for cls, style in class_to_style.items():
        content = re.sub(r'<div class="{}">'.format(re.escape(cls)), '<div style="{}">'.format(style), content)
    ...
    for cls, style in span_styles.items():
        content = re.sub(r'<span class="{}">'.format(re.escape(cls)), '<span style="{}">'.format(style), content)

    content = re.sub(r'<div class="flow-num">', '<div style="background:#2c5aa0;color:white;border-radius:50%;width:28px;height:28px;display:inline-flex;align-items:center;justify-content:center;font-weight:bold;font-size:0.9rem;flex-shrink:0;margin-right:12px;">', content)
    content = re.sub(r'<div class="flow-text">', '<div style="color:#333;flex:1;">', content)
    content = re.sub(r'<table class="drug-table">', '<table style="width:100%;border-collapse:collapse;margin:16px 0;font-size:0.95rem;">', content)
    content = re.sub(r'<details class="medical-details">', '<details style="background:#f8f9fa;margin:16px 0;border-radius:4px;border:1px solid #dee2e6;">', content)
    content = re.sub(r'<summary>', '<summary style="background:#2c5aa0;color:white;padding:12px;cursor:pointer;font-weight:bold;border-radius:4px 4px 0 0;">', content)
    content = re.sub(r'<div class="medical-details-content">', '<div style="padding:16px;background:white;border-radius:0 0 4px 4px;">', content)
    content = re.sub(r'<td>', '<td style="border:1px solid #ddd;padding:12px;text-align:left;">', content)

    content = '<div style="max-width:100%;padding:16px;line-height:1.7;color:#333;">\n' + content + '\n</div>'
    return content


def find_entry_url(env):
    """最新エントリからrt-PA記事のedit URLを取得"""
    ...
    for entry in entries:
        if "rt-PA" in entry or "時・血・画・出・偽" in entry:
            edit_match = re.search(r'<link rel="edit" href="([^"]+)"', entry)
            alt_match = re.search(r'<link rel="alternate"[^>]*href="([^"]+)"', entry)
            if edit_match:
                return edit_match.group(1), alt_match.group(1) if alt_match else ""
    return None, None
...
def main():
    ...
    # エントリ検索
    print("エントリ検索中...")
    edit_url, alt_url = find_entry_url(env)
    if not edit_url:
        print("[失敗] rt-PA記事のエントリが見つかりません")
        sys.exit(1)
    print("エントリ発見: {}".format(alt_url))
